...
<body class="app sidebar-mini rtl">
    ...
    <a href="inventory.php" class="btn btn-secondary">Back to Inventory</a>
        <a href="addequipment.php" class="btn btn-info">Add Equipment</a>
 ...
 <?php
    ...
    (e.equipment_name LIKE '%$searchKeyword%' OR e.equipment_desc LIKE '%$searchKeyword%' OR e.supplier_id LIKE '%$searchKeyword%' OR b.quantity_stock LIKE '%$searchKeyword%' OR b.price_punit LIKE '%$searchKeyword%' OR s.supplier_name LIKE '%$searchKeyword%' OR e.equipment_status LIKE '%$searchKeyword%')";

        $sql = "SELECT e.equipment_id, e.equipment_name, e.equipment_desc, e.equipment_status, e.supplier_id,
               ...
               COALESCE(b.price_punit, 'Batch not selected') as price_per_unit,
                e.equipment_status, s.supplier_name
        FROM equipment e
        LEFT JOIN supplier s ON e.supplier_id = s.supplier_id
        LEFT JOIN batch b ON e.equipment_id = b.equipment_id AND b.status = 'Used'" . $searchCondition . "
        GROUP BY e.equipment_id
        LIMIT $offset, $recordsPerPage";

        // Pagination query
$paginationSql = "SELECT COUNT(DISTINCT e.equipment_id) as total 
                  FROM equipment e
                  LEFT JOIN supplier s ON e.supplier_id = s.supplier_id
                  LEFT JOIN batch b ON e.equipment_id = b.equipment_id AND b.status = 'Used'" . $searchCondition;
        ...
        if (!$result) {
            die("Error: " . $conn->error);
                    ...
                    if (!empty($searchKeyword)) {
                        echo '<center><a href="equipment.php" class="btn btn-secondary mt-3">Back to All equipment</a></center>';
                    }
                    ?>
            ...
            <div class="container mt-3">
                <div class="table-responsive">
                    ...
                    if ($result->num_rows > 0) {
                        echo '<form method="get" action="equipment.php">';
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo '<td>' . $row['equipment_desc'] . '</td>';
                            echo '<td>' . $row['quantity_stock'] . '</td>';
                            echo '<td>' . $row['price_per_unit'] . '</td>';
                            echo '<td>' . $row['supplier_name'] . '</td>';
                            echo '<td>' . $row['equipment_status'] . '</td>';
                            echo '<td><a href="editequipment.php?id=' . $row['equipment_id'] . '" class="btn btn-primary">Edit</a></td>';
                            echo '<td><a href="batchequipment.php?id=' . $row['equipment_id'] . '" class="btn btn-primary">BatchList</a></td>';
                            echo '</tr>';
                            $rowNumber++;
                        }
                        ...
                        echo '<p class="alert alert-info">No Equipment found.</p>';
